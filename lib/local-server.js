...
function createRequestHandler(workspaceDir, htmlContent, opts = {}) {
  ...
  return async function handler(req, res) {
    ...
    // Backend sources — authoritative list of files already synced to Echo cloud
    if (url.pathname === "/api/backend-sources") {
      ...
      try {
        ...
        // Build a map of file_path → { is_processed, created_at, id }
        ...
        sendJson(res, {
          ok: true,
          totalSources: status.total_source_versions || 0,
          processedSources: status.processed_source_versions || 0,
          recentMemoryCount: status.recent_memory_count || 0,
          sources,
        });
      } catch (e) {
        sendJson(res, { ok: false, sources: [], error: String(e?.message ?? e) });
      }
      return;
    }
    ...
    if (url.pathname === "/api/sync-selected" && req.method === "POST") {
      ...
      try {
        ...
        // Convert relative paths to absolute paths for filtering
        const absSet = new Set(paths.map(p => path.resolve(workspaceDir, p)));
        const result = await syncRunner.runSync("local-ui-selected", absSet);
        sendJson(res, result);
      } catch (e) {
        res.writeHead(500, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: String(e?.message ?? e) }));
