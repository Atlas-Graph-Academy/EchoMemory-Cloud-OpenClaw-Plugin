...
// Keep in sync with SKIP_DIRS in openclaw-memory-scan.js.
const SKIP_DIRS = new Set([
  "node_modules", ".git", ".next", "dist", "build", "__pycache__",
  "logs", "completions", "delivery-queue", "browser", "canvas", "cron", "media",
  "_disabled-plugins", "state", "extensions", "credentials", "devices",
  "flows", "identity", "locks",
]);
  ...
  const fileStatuses = files.map((f) => {
    ...
    if (isPrivate) {
      ...
      if (trustForUpdate) {
        ...
        // Only flag `needsUserNotice` when there's actually something to
        // notice — i.e. content has changed since the last sync. A trusted
        // private file that's still in sync doesn't belong in the updates
        // panel (nothing for the user to do).
        const contentChangedSinceSync = successfulHash !== f.contentHash;
        return {
          fileName: f.fileName,
          ...
          autoSyncOk: false,
          ...(contentChangedSinceSync ? { needsUserNotice: true } : {}),
          changedSections,
          lastError,
          lastAttemptAt,
          lastSuccessAt,
    ...
    // `review` privacyLevel (MEMORY.md, AGENTS.md) only earns the
    // "needs user attention" treatment AFTER it's been synced once. A
    // never-synced review file is just a new file waiting for first
    // upload — putting it in the updates badge would clutter the panel
    // with files that have no prior version to diff against. The
    // distinction:
    //   - Review + previously-synced + content changed: badge it,
    //     keep auto-sync off so OpenClaw rewrites surface for review.
    //   - Review + never synced (or unchanged since last sync): treat
    //     as a regular eligible file — auto-sync picks it up like any
    //     safe markdown.
    const isReview = f.privacyLevel === "review";
    const previouslySynced = Boolean(successfulHash);
    const contentChangedSinceSync = previouslySynced && successfulHash !== f.contentHash;
    const reviewNeedsAttention = isReview && contentChangedSinceSync;
    ...
    return {
      ...
      syncEligible: true,
      syncReason: "eligible",
      autoSyncOk: !reviewNeedsAttention,
      ...(reviewNeedsAttention ? { needsUserNotice: true } : {}),
      changedSections,
      lastError,
      lastAttemptAt,
      lastSuccessAt,
...
function createRequestHandler(workspaceDir, htmlContent, opts = {}) {
  ...
  return async function handler(req, res) {
    ...
    // Serve static assets from dist/. Vite drops public/ subdirs (rive/,
    // fonts/, icons/) at the dist root, not under /assets/, so we need to
    // accept those prefixes too — otherwise every .riv fetch 404s and the
    // entire UI loses its Rive animations.
    if (
      ...
      try {
        ...
        // SVG was the canary here — browsers happily sniff PNG even when
        // Content-Type is wrong, but they refuse to render SVG unless it's
        // explicitly image/svg+xml. The previous octet-stream fallback was
        // why ChatGPT's logo "kept disappearing".
        const mimeTypes = {
        ...
        // charset is meaningful only for text-shaped content types.
        // Appending it to image/png etc. is technically wrong (browsers
        // tolerate it, but linters/proxies may complain).
    ...
    if (url.pathname === "/api/auth/verify-otp" && req.method === "POST") {
      try {
        ...
        // Intentionally NOT auto-firing a sync after login. Two reasons:
        // (1) E2EE accounts have not yet unlocked their key at this point, so
        //     every file fails with ENCRYPTION_KEY_REQUIRED. (2) Unbounded
        //     auto-sync of hundreds of files surprises the user. The local UI
        //     refetches /api/sync-status; the user clicks Sync when ready.
    ...
    if (url.pathname === "/api/encryption-setup" && req.method === "POST") {
      try {
        ...
        const passphrase = typeof bodyResult.body?.passphrase === "string"
          ? bodyResult.body.passphrase
          : "";
        // Passphrase contract: at least 5 visible characters, no whitespace.
        // The plugin UI (and the Chrome extension) accept letters, digits,
        // and symbols — server treats it as an opaque string for PBKDF2,
        // so any client that produces a matching derived key works.
        // Whitespace is rejected because leading/trailing-space typos are
        // a common silent mismatch source on copy-paste.
        if (typeof passphrase !== "string" || passphrase.length < 5 || /\s/.test(passphrase)) {
          sendJsonWithStatus(res, 400, {
            ok: false,
            error: "Passphrase must be at least 5 characters with no spaces",
          });
          return;
        }
        const who = await activeApiClient.whoami();
        ...
        if (existing?.enabled) {
          sendJsonWithStatus(res, 409, {
            ok: false,
            error: "Encryption is already enabled for this account. Unlock with your existing passphrase instead.",
          });
          return;
        }
    ...
    if (url.pathname === "/api/sync-cancel" && req.method === "POST") {
      if (!syncRunner) {
        res.writeHead(400, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: "Sync not available" }));
        return;
      }
      const cancelled = syncRunner.cancelRun?.() === true;
      sendJson(res, {
        ok: true,
        cancelled,
        activeRun: syncRunner.getActiveRunInfo?.() ?? null,
