  ...
  return async function handler(req, res) {
    ...
    if (url.pathname === "/api/sync-status") {
      try {
        ...
        const [lastState, files, importStatus] = await Promise.all([
          statePath ? readLastSyncState(statePath) : Promise.resolve(null),
          scanFullWorkspace(workspaceDir),
          apiClient ? apiClient.getImportStatus().catch(() => null) : Promise.resolve(null),
        ]);
        ...
        if (Array.isArray(lastState?.results)) {
          for (const r of lastState.results) {
            if (r.filePath && r.contentHash) {
              lastSyncedMap.set(r.filePath, r.contentHash);
            }
          }
        }
        ...
        const fileStatuses = files.map((f) => {
          ...
          const isSyncable =
            f.relativePath.startsWith("memory/") &&
          ...
          if (lastSyncedMap.has(absPath)) {
            const status = lastSyncedMap.get(absPath) === f.contentHash ? "synced" : "modified";
            return {
              fileName: f.fileName,
        ...
        sendJson(res, {
          ...
          totalSyncedSources: importStatus?.total_source_versions ?? 0,
          fileStatuses,
        });
      } catch (err) {
        res.writeHead(500, { "Content-Type": "application/json" });
