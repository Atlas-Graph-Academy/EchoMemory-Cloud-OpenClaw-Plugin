...
function runOpenClawCommand(args, logger) {
  return new Promise((resolve, reject) => {
    const child = spawn(getOpenClawCommand(), args, {
      cwd: PLUGIN_ROOT_DIR,
      stdio: "pipe",
      windowsHide: true,
    });
...
async function performPluginUpdate(logger, { restartGateway = false } = {}) {
  ...
  _pluginUpdatePromise = (async () => {
    ...
    const output = await runOpenClawCommand(["plugins", "install", installSpec], logger);
    ...
    return {
      ...
      message: restartGateway
        ? `Installed ${installSpec} and triggered openclaw gateway restart.`
        : `Installed ${installSpec}. Restart openclaw gateway to load the new version.`,
    };
  })();
...
function createRequestHandler(workspaceDir, htmlContent, opts = {}) {
  ...
  return async function handler(req, res) {
    ...
    if (url.pathname === "/api/cloud-memories") {
      ...
      try {
        const data = await apiClient.listCloudMemories({ limit: 250, offset: 0 });
        sendJson(res, {
          ok: true,
    ...
    if (url.pathname === "/api/cloud-sources") {
      ...
      try {
        const data = await apiClient.listCloudSources({ limit: 250, offset: 0 });
        sendJson(res, {
          ok: true,
          data: Array.isArray(data?.data) ? data.data : [],
          count: Number.isFinite(data?.count) ? data.count : 0,
