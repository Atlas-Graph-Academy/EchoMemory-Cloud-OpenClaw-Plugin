  ...
  return async function handler(req, res) {
    ...
    if (url.pathname === "/api/setup-config" && req.method === "POST") {
      try {
        ...
        if (cfg) {
          cfg.baseUrl = payload.ECHOMEM_BASE_URL.trim()
            ? payload.ECHOMEM_BASE_URL.trim().replace(/\/+$/, "")
            : cfg.baseUrl;
          cfg.webBaseUrl = payload.ECHOMEM_WEB_BASE_URL.trim()
            ? payload.ECHOMEM_WEB_BASE_URL.trim().replace(/\/+$/, "")
            : cfg.webBaseUrl;
