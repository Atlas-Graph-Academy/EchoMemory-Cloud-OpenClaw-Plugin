...
async function buildWorkspaceSyncView({ workspaceDir, syncMemoryDir, statePath, apiClient, logger }) {
  const backendSourcesPromise = readBackendSourceMap(apiClient, workspaceDir).catch(() => new Map());
  let [lastState, files, backendSources] = await Promise.all([
  ...
  // Account-switch defense: the sync-state file records per-file status
  // tied to the Echo user that produced it. If the logged-in user now
  // differs, none of those statuses describe the new user's bucket, so
  // treat the state as empty and remove the file so later writes stamp
  // the correct userId.
  let currentUserId = apiClient?.getLastKnownUserId?.() ?? null;
  if (!currentUserId && apiClient?.whoami) {
    try {
      const whoami = await Promise.race([
        apiClient.whoami(),
        resolveWithin(BACKEND_SOURCE_LOOKUP_TIMEOUT_MS, null),
      ]);
      currentUserId = whoami?.user_id ?? apiClient?.getLastKnownUserId?.() ?? null;
    } catch {
      // whoami failures are non-fatal; skip the mismatch check this round.
    }
  }
  if (lastState?.userId && currentUserId && lastState.userId !== currentUserId) {
    logger?.info?.(
      `[echo-memory] sync state belongs to user ${lastState.userId}, current user is ${currentUserId}; clearing stale state`,
    );
    if (statePath) {
      try {
        await clearLastSyncState(statePath);
      } catch (err) {
        logger?.warn?.(`[echo-memory] failed to clear stale sync state: ${String(err?.message ?? err)}`);
      }
    }
    lastState = null;
  }
  ...
  return async function handler(req, res) {
    ...
    if (url.pathname === "/api/auth/verify-otp" && req.method === "POST") {
      try {
        ...
        // If the verified account differs from whoever owned the existing
        // sync state, wipe the state file so the new account starts from a
        // clean slate (its bucket has none of those imports).
        const newUserId = result.user_id || null;
        const statePath = opts.syncRunner?.getStatePath?.() ?? null;
        if (statePath && newUserId) {
          try {
            const existing = await readLastSyncState(statePath);
            if (existing?.userId && existing.userId !== newUserId) {
              await clearLastSyncState(statePath);
              logger?.info?.(
                `[echo-memory] account switched from ${existing.userId} to ${newUserId}; cleared sync state`,
              );
            }
          } catch (err) {
            logger?.warn?.(
              `[echo-memory] failed to reconcile sync state after login: ${String(err?.message ?? err)}`,
            );
          }
        }
    ...
    if (url.pathname === "/api/setup-config" && req.method === "POST") {
      try {
        ...
        // Disconnecting (apiKey cleared) means the user is logging out of
        // this Echo account on this device. Wipe per-user sync history so
        // the next account to log in sees a clean slate and doesn't inherit
        // fake synced/failed statuses from the previous owner's bucket.
        const apiKeyWasCleared = !payload.ECHOMEM_API_KEY.trim() && cfg?.apiKey;
        if (apiKeyWasCleared) {
          const statePath = opts.syncRunner?.getStatePath?.() ?? null;
          if (statePath) {
            try {
              await clearLastSyncState(statePath);
              logger?.info?.(`[echo-memory] cleared sync state on disconnect`);
            } catch (err) {
              logger?.warn?.(
                `[echo-memory] failed to clear sync state on disconnect: ${String(err?.message ?? err)}`,
              );
            }
          }
          apiClient?.setLastKnownUserId?.(null);
        }
        if (cfg) {
