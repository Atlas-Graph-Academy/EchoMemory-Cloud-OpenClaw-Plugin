...
function normalizeEncryptionKeyBase64(value) {
  const normalized = typeof value === "string" ? value.trim() : "";
  if (!normalized || normalized.length < 40 || normalized.length > 50) {
    return null;
  }
  return /^[A-Za-z0-9+/]+={0,2}$/.test(normalized) ? normalized : null;
}

function readEncryptionKeyFromRequest(req, body = {}) {
  return normalizeEncryptionKeyBase64(
    body?.encryptionKeyBase64
      ?? body?.encryptionKey
...
async function buildWorkspaceSyncView({ workspaceDir, syncMemoryDir, statePath, apiClient }) {
  ...
  const fileStatuses = files.map((f) => {
    ...
    // Any .md file anywhere under the memory dir subtree is a sync target.
    // (Private paths and sensitive content are still filtered out by the
    // isPrivate branch above / the sensitive-content scan.)
    const isSyncTarget =
      Boolean(syncMemoryDir)
      && (absPath + path.sep).startsWith(syncMemoryDir + path.sep);
  ...
  return async function handler(req, res) {
    ...
    if (req.method === "OPTIONS") {
      res.writeHead(204, {
        ...
        "Access-Control-Allow-Headers": "Content-Type, X-Encryption-Key",
      });
      res.end();
      return;
    }
    ...
    if (url.pathname === "/api/auth/verify-otp" && req.method === "POST") {
      try {
        ...
        const bootstrap = typeof activeApiClient.getAccountBootstrap === "function"
          ? await activeApiClient.getAccountBootstrap().catch(() => null)
          : null;
    ...
    if (url.pathname === "/api/account-bootstrap") {
      try {
        const activeApiClient = resolveApiClient();
        if (!activeApiClient || !cfg?.apiKey) {
          sendJsonWithStatus(res, 401, { ok: false, error: "Echo API key is required" });
          return;
        }
        ...
        if (req.method === "GET") {
          sendJson(res, {
            ok: true,
            bootstrap: await activeApiClient.getAccountBootstrap(),
          });
          return;
        }
        ...
        if (req.method === "POST") {
          ...
          const preference = bodyResult.body?.encryptionPreference;
          const result = await activeApiClient.completeAccountBootstrap({
            echoName: typeof bodyResult.body?.echoName === "string" ? bodyResult.body.echoName : "Echo",
            encryptionPreference: preference === "maximum" || preference === "standard" ? preference : undefined,
          });
          sendJson(res, { ok: true, bootstrap: result });
          return;
        }
      } catch (error) {
        sendJsonWithStatus(res, normalizeErrorStatus(error), {
    ...
    if (url.pathname === "/api/account/encryption") {
      try {
        const activeApiClient = resolveApiClient();
        if (!activeApiClient || !cfg?.apiKey) {
          sendJsonWithStatus(res, 401, { ok: false, error: "Echo API key is required" });
        ...
        if (req.method === "GET") {
          sendJson(res, {
            ok: true,
            encryption: await activeApiClient.getEncryptionConfig(),
          });
          return;
        }
        ...
        if (req.method === "POST") {
          ...
          const salt = typeof bodyResult.body?.salt === "string" ? bodyResult.body.salt.trim() : "";
          const verification = typeof bodyResult.body?.verification === "string" ? bodyResult.body.verification.trim() : "";
          const parsedIterations = Number.parseInt(String(bodyResult.body?.iterations ?? 600000), 10);
          const iterations = Number.isFinite(parsedIterations) ? parsedIterations : 600000;
          if (!salt || !verification) {
            sendJsonWithStatus(res, 400, { ok: false, error: "Salt and verification token are required" });
            return;
          }
          const result = await activeApiClient.setupEncryptionConfig({ salt, verification, iterations });
          sendJson(res, { ok: true, encryption: result });
          return;
        }
      } catch (error) {
        sendJsonWithStatus(res, normalizeErrorStatus(error), {
    ...
    if (url.pathname === "/api/setup-config" && req.method === "POST") {
      try {
        ...
        if (!bodyResult.ok) {
          logger?.warn?.(
            `[echo-memory] invalid JSON for ${url.pathname}: ${bodyResult.parseError}`,
          );
          res.writeHead(400, { "Content-Type": "application/json; charset=utf-8" });
    ...
    if (url.pathname === "/api/sync" && req.method === "POST") {
      ...
      try {
        ...
        if (!bodyResult.ok) {
          res.writeHead(400, { "Content-Type": "application/json" });
          res.end(JSON.stringify({
            error: "Invalid JSON body for sync",
            details: bodyResult.parseError,
          }));
          return;
        }
        const encryptionKeyBase64 = readEncryptionKeyFromRequest(req, bodyResult.body);
        const result = await syncRunner.runSync("local-ui", null, { encryptionKeyBase64 });
        sendJson(res, result);
      } catch (e) {
        res.writeHead(500, { "Content-Type": "application/json" });
    ...
    if (url.pathname === "/api/sync-selected" && req.method === "POST") {
      ...
      try {
        ...
        const body = bodyResult.body;
        const encryptionKeyBase64 = readEncryptionKeyFromRequest(req, body);
        ...
        if (requestedInvalidPaths.length > 0) {
          ...
          res.end(JSON.stringify({
            ...
            details: "Selected files must be markdown files inside the configured memory directory.",
    ...
    if (url.pathname === "/api/reextract-selected" && req.method === "POST") {
      ...
      try {
        ...
        const result = await syncRunner.runSync(
          "local-ui-reextract",
          requestedFilterPaths,
          { force: true, encryptionKeyBase64 },
        );
        sendJson(res, result);
        return;
      } catch (e) {
