...
async function buildWorkspaceSyncView({ workspaceDir, syncMemoryDir, statePath, apiClient }) {
  ...
  const fileStatuses = files.map((f) => {
    ...
    if (isPrivate) {
      // Private/sensitive content is not sync-eligible by default, but the
      // user can override with explicit confirmation. The frontend reads
      // `confirmable: true` to surface the "are you sure?" prompt; on
      // accept it sends `confirmPrivate: true` to the sync-selected
      // endpoint, which then accepts these paths despite syncEligible=false.
      return {
        fileName: f.fileName,
