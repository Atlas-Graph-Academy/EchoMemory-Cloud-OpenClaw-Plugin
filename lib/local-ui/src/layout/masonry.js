 ...
 */
export function cardSize(file, contentLen) {
  ...
  // Approximate: ~60 chars per line, ~16px per line, capped
...
function masonryPack(items, maxWidth, defaultColWidth, contentMap) {
  ...
  for (const item of items) {
    ...
    const { w, h } = cardSize(item, contentLen);
    const isLong = (item.sizeBytes >= EXCLUSIVE_THRESHOLD);
...
export function computeLayout(files, _vpWidth = 1400, contentMap = null) {
  ...
  for (const f of allFiles) {
    ...
    const { w, h } = cardSize(f, contentLen);
