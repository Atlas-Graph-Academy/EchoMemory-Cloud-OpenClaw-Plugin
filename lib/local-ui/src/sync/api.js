...
export async function fetchEncryptionState() {
  try {
    const res = await fetch('/api/encryption-state');
    if (!res.ok) return { connected: false, enabled: false, unlocked: false };
    return await res.json();
  } catch {
    return { connected: false, enabled: false, unlocked: false };
  }
}
...
export async function unlockEncryption(passphrase) {
  ...
  const data = await res.json().catch(() => ({}));
  if (!res.ok || data?.ok === false) {
    throw new Error(data?.error || `Unlock failed: HTTP ${res.status}`);
  }
  return data;
}

export async function lockEncryption() {
  const res = await fetch('/api/encryption-lock', { method: 'POST' });
  const data = await res.json().catch(() => ({}));
  if (!res.ok || data?.ok === false) {
    throw new Error(data?.error || `Lock failed: HTTP ${res.status}`);
  }
  return data;
}
...
export async function fetchFileDiff(relativePath) {
  const res = await fetch(`/api/file-diff?path=${encodeURIComponent(relativePath)}`);
  const data = await res.json().catch(() => ({}));
  if (!res.ok || data?.ok === false) {
    const err = new Error(data?.error || `File diff failed: HTTP ${res.status}`);
...
export async function cancelSync() {
  ...
  const data = await res.json().catch(() => ({}));
  if (!res.ok) throw new Error(data?.details || data?.error || `Cancel failed: HTTP ${res.status}`);
  return data;
}
...
export async function triggerSyncSelected(paths, { confirmPrivate = false } = {}) {
  const res = await fetch('/api/sync-selected', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ paths, confirmPrivate }),
  });
  const data = await res.json().catch(() => ({}));
  // 409 with requiresConfirmation is a soft block — the caller should
  // surface a confirmation prompt and retry with confirmPrivate: true.
  // Surface the structured payload via err.payload so handlers can branch.
  if (!res.ok) {
    const err = new Error(data?.details || data?.error || `Sync failed: HTTP ${res.status}`);
    err.status = res.status;
    err.payload = data || null;
    throw err;
  }
