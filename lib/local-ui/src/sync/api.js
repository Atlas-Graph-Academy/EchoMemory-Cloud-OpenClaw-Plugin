...
/**
 * Fetch content for all files in parallel (batched).
 * Returns a Map<relativePath, content>.
 */
export async function fetchAllContents(files) {
  const map = new Map();
  // Batch in groups of 20 to avoid hammering
  const BATCH = 20;
  for (let i = 0; i < files.length; i += BATCH) {
    const batch = files.slice(i, i + BATCH);
    const results = await Promise.all(
      batch.map(f => fetchFileContent(f.relativePath))
    );
    for (let j = 0; j < batch.length; j++) {
      const r = results[j];
      if (r && r.content != null) {
        map.set(batch[j].relativePath, r.content);
      }
    }
  }
  return map;
