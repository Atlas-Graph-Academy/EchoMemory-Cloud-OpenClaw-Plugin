...
export async function completeAccountBootstrap({ echoName = 'Echo', encryptionPreference } = {}) {
  const res = await fetch('/api/account-bootstrap', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ echoName, encryptionPreference }),
  });
  const data = await res.json().catch(() => ({}));
  if (!res.ok || data?.ok === false) {
...
export async function saveEncryptionConfig({ salt, verification, iterations }) {
  const res = await fetch('/api/account/encryption', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  const data = await res.json().catch(() => ({}));
  if (!res.ok || data?.ok === false) {
    throw new Error(data?.error || `Encryption setup failed: HTTP ${res.status}`);
  }
  return data?.encryption || data;
}

function buildEncryptionBody(encryptionKeyBase64, extra = {}) {
...
export async function triggerSync({ encryptionKeyBase64 = null } = {}) {
  const res = await fetch('/api/sync', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: buildEncryptionBody(encryptionKeyBase64),
  });
  const data = await res.json().catch(() => ({}));
...
export async function triggerSyncSelected(paths, { encryptionKeyBase64 = null } = {}) {
  const res = await fetch('/api/sync-selected', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: buildEncryptionBody(encryptionKeyBase64, { paths }),
  });
  const data = await res.json().catch(() => ({}));
  if (!res.ok) throw new Error(data?.details || data?.error || `Sync failed: HTTP ${res.status}`);
  return data;
...
export async function triggerReextractSelected(paths, { encryptionKeyBase64 = null } = {}) {
