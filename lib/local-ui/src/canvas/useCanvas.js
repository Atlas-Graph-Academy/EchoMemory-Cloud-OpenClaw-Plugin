...
export function useCanvasTransform(viewportRef, canvasRef, bounds) {
  ...
  const onWheel = useCallback((e) => {
    ...
    // Shift+wheel → horizontal pan (standard).
    ...
    // Trackpad two-finger scroll (pan): no ctrlKey, small deltas.
    // ctrlKey is set by the OS for pinch-to-zoom gestures.
    // Mouse wheels produce large discrete deltas (≥50).
    const isMouseWheel = !e.ctrlKey && Math.abs(e.deltaY) >= 50;
    const isPinchZoom = e.ctrlKey;
    const isTrackpadScroll = !e.ctrlKey && !isMouseWheel;

    if (isTrackpadScroll) {
      // Cancel any running zoom animation so pan takes over
      if (rafRef.current != null && wheelTargetRef.current) {
        cancelAnimationFrame(rafRef.current);
        rafRef.current = null;
        wheelTargetRef.current = null;
      }
      const { panX, panY } = liveRef.current;
      liveRef.current.panX = panX - e.deltaX;
      liveRef.current.panY = panY - e.deltaY;
      applyTransform();
      syncState(80);
      return;
    }
    ...
    // If an explicit animation (focusCard, zoomIn/Out, spring-back) owns the
    // RAF, cancel it so the wheel takes over immediately.
    if (rafRef.current != null && !wheelTargetRef.current) {
      cancelAnimationFrame(rafRef.current);
      rafRef.current = null;
    }
