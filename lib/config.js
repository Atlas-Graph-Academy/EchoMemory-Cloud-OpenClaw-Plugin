...
const LOCAL_UI_MANAGED_FIELDS = new Set([
  "apiKey",
  "memoryDir",
  "localOnlyMode",
  "autoSync",
...
function resolveConfigValue(pluginConfig, configKey, envKey, fallback, { preferEnv = false } = {}) {
  const env = loadEnvFiles();
  if (preferEnv) {
...
function resolveRuntimeConfigValue(pluginConfig, configKey, envKey, fallback) {
  return resolveConfigValue(
    pluginConfig,
    configKey,
    envKey,
    fallback,
    { preferEnv: LOCAL_UI_MANAGED_FIELDS.has(configKey) },
  );
}
...
export function getLocalUiSetupState(pluginConfig = {}, cfg = null) {
  ...
  const localOnlyMode = resolveRuntimeConfigValue(
    pluginConfig,
    "localOnlyMode",
  ...
  const apiKey = runtimeCfg.localOnlyMode
    ...
    : resolveRuntimeConfigValue(pluginConfig, "apiKey", "ECHOMEM_API_KEY", runtimeCfg.apiKey);
  const memoryDir = resolveRuntimeConfigValue(pluginConfig, "memoryDir", "ECHOMEM_MEMORY_DIR", runtimeCfg.memoryDir);
  const autoSync = resolveRuntimeConfigValue(
    pluginConfig,
    "autoSync",
    "ECHOMEM_AUTO_SYNC",
    runtimeCfg.autoSync,
  );
  const syncIntervalMinutes = resolveRuntimeConfigValue(
    pluginConfig,
    "syncIntervalMinutes",
    "ECHOMEM_SYNC_INTERVAL_MINUTES",
    runtimeCfg.syncIntervalMinutes,
  );
  const batchSize = resolveRuntimeConfigValue(
    pluginConfig,
    "batchSize",
    "ECHOMEM_BATCH_SIZE",
    runtimeCfg.batchSize,
  );
  const requestTimeoutMs = resolveRuntimeConfigValue(
    pluginConfig,
    "requestTimeoutMs",
    "ECHOMEM_REQUEST_TIMEOUT_MS",
    runtimeCfg.requestTimeoutMs,
  );
  const disableOpenClawMemoryToolsWhenConnected = resolveRuntimeConfigValue(
    pluginConfig,
    "disableOpenClawMemoryToolsWhenConnected",
...
export function buildConfig(pluginConfig = {}) {
  ...
  const localOnlyModeSetting = resolveRuntimeConfigValue(cfg, "localOnlyMode", "ECHOMEM_LOCAL_ONLY_MODE", false);
  const localOnlyMode = parseBoolean(localOnlyModeSetting.value, false);
  const disableOpenClawMemoryToolsSetting = resolveRuntimeConfigValue(
    cfg,
    "disableOpenClawMemoryToolsWhenConnected",
    DISABLE_OPENCLAW_MEMORY_TOOLS_ENV_KEY,
    false,
  );
  const apiKeySetting = resolveRuntimeConfigValue(cfg, "apiKey", "ECHOMEM_API_KEY", "");
  const autoSyncSetting = resolveRuntimeConfigValue(cfg, "autoSync", "ECHOMEM_AUTO_SYNC", true);
  const syncIntervalSetting = resolveRuntimeConfigValue(
    cfg,
    "syncIntervalMinutes",
    "ECHOMEM_SYNC_INTERVAL_MINUTES",
    MIN_AUTO_SYNC_INTERVAL_MINUTES,
  );
  const batchSizeSetting = resolveRuntimeConfigValue(cfg, "batchSize", "ECHOMEM_BATCH_SIZE", 10);
  const requestTimeoutSetting = resolveRuntimeConfigValue(
    cfg,
    "requestTimeoutMs",
    "ECHOMEM_REQUEST_TIMEOUT_MS",
    300000,
  );
  const memoryDirSetting = resolveRuntimeConfigValue(
    cfg,
    "memoryDir",
    "ECHOMEM_MEMORY_DIR",
  ...
  return {
    ...
    apiKey: localOnlyMode ? "" : String(apiKeySetting.value || "").trim(),
    localOnlyMode,
    disableOpenClawMemoryToolsWhenConnected: parseBoolean(disableOpenClawMemoryToolsSetting.value, false),
    autoSync: parseBoolean(autoSyncSetting.value, true),
    syncIntervalMinutes: parseInteger(
      syncIntervalSetting.value,
      MIN_AUTO_SYNC_INTERVAL_MINUTES,
      { min: MIN_AUTO_SYNC_INTERVAL_MINUTES, max: 1440 },
    ),
    batchSize: parseInteger(batchSizeSetting.value, 10, { min: 1, max: 25 }),
    requestTimeoutMs: parseInteger(
      requestTimeoutSetting.value,
      300000,
      { min: 1000, max: 900000 },
    ),
