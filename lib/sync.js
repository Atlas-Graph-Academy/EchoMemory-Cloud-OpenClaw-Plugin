...
export function createSyncRunner({ api, cfg, client, fallbackStateDir = null, stableStateDir = null }) {
  ...
  // Stamp every state write with the current Echo user id so a later read
  // can detect account switches and discard stale history. The userId may
  // be null on the very first sync after installation; subsequent writes
  // catch up once whoami/verifyOtp has populated the cache.
  async function writeStateStamped(state) {
    const stamped = {
      ...state,
      userId: client?.getLastKnownUserId?.() ?? state?.userId ?? null,
    };
    await writeLastSyncState(getStatePath(), stamped);
  }
