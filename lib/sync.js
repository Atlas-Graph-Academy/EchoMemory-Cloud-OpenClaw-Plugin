...
const AUTO_SYNC_TRIGGERS = new Set(["startup", "scheduled", "compat-startup"]);
...
function isAutoSyncTrigger(trigger) {
  return AUTO_SYNC_TRIGGERS.has(String(trigger || "").trim().toLowerCase());
}

function shouldThrottleAutoSync(prevState, trigger, intervalMs) {
  if (!isAutoSyncTrigger(trigger)) {
    return false;
  }
...
export function createSyncRunner({ api, cfg, client, fallbackStateDir = null, stableStateDir = null }) {
  ...
  async function runSync(trigger = "manual", filterPaths = null) {
    ...
    activeRun = (async () => {
      ...
      try {
        if (shouldThrottleAutoSync(prevState, trigger, intervalMs)) {
          api.logger?.info?.(
        ...
        let scannedFiles = [];
        try {
          scannedFiles = await scanOpenClawMemoryDir(cfg.memoryDir);
        } catch (error) {
        ...
        const batches = chunkFiles(files, cfg.batchSize);
  ...
  function startInterval() {
    stopInterval();
    const intervalMs = Math.max(MIN_AUTO_SYNC_INTERVAL_MINUTES, cfg.syncIntervalMinutes) * 60 * 1000;
