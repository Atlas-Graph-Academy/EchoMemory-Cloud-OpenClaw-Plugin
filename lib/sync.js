import { classifyFile, scanOpenClawMemoryDir, scanSelectedMarkdownFiles } from "./openclaw-memory-scan.js";
import {
  migrateStateFile,
  resolveStatePath,
  resolveDefaultStatePath,
  adoptLegacyStateForAccount,
  readLastSyncState,
  writeLastSyncState,
} from "./state.js";
import * as encryptionCache from "./encryption-cache.js";

function buildAccountTag(whoamiResult) {
  const userId = whoamiResult?.user_id ? String(whoamiResult.user_id) : null;
  const email = typeof whoamiResult?.email === "string" ? whoamiResult.email : null;
...
// Triggers that run on the user's behalf without per-file selection (timers,
// watchers, login replay, the "Sync" button). For these, only `safe` files
// auto-flow. `review` (MEMORY.md/AGENTS.md) and `private` (SOUL.md/USER.md/
// IDENTITY.md/TOOLS.md and **/private/**) are deliberately held back so the
// user explicitly clicks them in the directory after seeing what changed —
// no silent replace of profile-class content.
const NON_TARGETED_TRIGGERS = new Set([
  "startup",
  "compat-startup",
  "scheduled",
  "file-change",
  ...
  const lastSuccessfulContentHash =
    status === "failed"
      ? previousResult?.lastSuccessfulContentHash
        ...
        ?? null
      : file.contentHash;
  // Snapshot per-section hashes when sync succeeds. The local-server's
  // buildWorkspaceSyncView compares the live file's section hashes against
  // this snapshot to surface "Mission and Values changed since last sync"
  // — no diff content stored locally; the cloud's source_of_truth has
  // the prior content if the user opens the diff modal.
  const lastSuccessfulSectionHashes =
    status === "failed"
      ? previousResult?.lastSuccessfulSectionHashes
        ?? previousResult?.last_successful_section_hashes
        ?? null
      : (file.sectionHashes && typeof file.sectionHashes === "object"
        ? file.sectionHashes
        : null);
...
export function createSyncRunner({ api, cfg, client, workspaceDir = null, fallbackStateDir = null, stableStateDir = null }) {
  ...
  async function initialize(overrideStateDir) {
    const { primaryStateDir, legacyStateDirs } = resolveStateDirectories(overrideStateDir);
    if (!primaryStateDir) {
      throw new Error("Echo memory state directory is unavailable");
    }
    stateDir = primaryStateDir;

    const defaultPath = resolveDefaultStatePath(primaryStateDir);
    const migratedFrom = await migrateStateFile(
      defaultPath,
      legacyStateDirs.map((legacyStateDir) => resolveDefaultStatePath(legacyStateDir)),
    );
    if (migratedFrom) {
      api.logger?.info?.(`[echo-memory] migrated sync state from ${migratedFrom} to ${defaultPath}`);
    }

    primeAccountIdInBackground();
  }

  function resolveStateDirOrThrow() {
  ...
  function getStatePath() {
    const dir = resolveStateDirOrThrow();
    return resolveStatePath(dir, currentAccountId);
  }
  ...
  async function setCurrentAccountId(accountId) {
    const next = accountId ? String(accountId) : null;
    if (next === currentAccountId) return;
    currentAccountId = next;
    if (next && stateDir) {
      try {
        const adopted = await adoptLegacyStateForAccount(stateDir, next);
        if (adopted) {
          api.logger?.info?.(
            `[echo-memory] adopted legacy sync state for account ${next.slice(0, 8)}…`,
          );
        }
      } catch (err) {
        api.logger?.warn?.(
          `[echo-memory] legacy state adoption failed: ${String(err?.message ?? err)}`,
        );
      }
    }
  }

  function primeAccountIdInBackground() {
    if (currentAccountId) return;
    if (!cfg?.apiKey || typeof client?.whoami !== "function") return;
    client.whoami()
      .then((result) => {
        if (result?.user_id) {
          setCurrentAccountId(String(result.user_id)).catch(() => {});
        }
      })
      .catch(() => {
        // whoami failure during priming is non-fatal — accountId stays null
        // until the next explicit sync or API-key change.
      });
  }
      ...
      try {
        ...
        if (!cfg.apiKey) {
          ...
          const state = {
            ...
            results: prevResults,
            run_results: [],
            account: prevState?.account ?? null,
          };
        ...
        currentAccount = buildAccountTag(whoamiResult);

        if (currentAccount?.userId && currentAccount.userId !== currentAccountId) {
          await setCurrentAccountId(currentAccount.userId);
          // Re-read state under the correct per-account file now that we know
          // which account this sync belongs to. Previously-loaded prevResults
          // came from the default/unknown file and may not apply.
          try {
            prevState = await readLastSyncState(getStatePath());
            prevResults = Array.isArray(prevState?.results) ? prevState.results : [];
            resultMap.clear();
        ...
        // E2EE gate — fail-closed before doing any work. If the account has
        // encryption enabled, the plugin must have a cached derived key for
        // this user (set via /api/encryption-unlock). Without it we cannot
        // safely upload anything without leaking plaintext (the audit found
        // the upstream import-markdown route would silently degrade).
        let encryptionKey = null;
        if (currentAccount?.userId) {
          const encConfig = await client.getEncryptionConfig().catch(() => null);
          if (encConfig?.enabled) {
            encryptionKey = encryptionCache.getKey(currentAccount.userId);
            if (!encryptionKey) {
              const message =
                "Encryption is enabled for this account but the plugin is locked. Open the panel and unlock with your passphrase before syncing.";
              emitProgress(buildProgressPayload({
                phase: "failed",
                runId,
                trigger,
        ...
        // Scan the broader workspace, not just memoryDir. Lets users sync
        // markdown that lives outside ECHOMEM_MEMORY_DIR (e.g. notes elsewhere
        // in the OpenClaw workspace). The scanner already skips `private/`
        // dirs and `private-*.md` files, so privacy guarantees are preserved.
        // Falls back to memoryDir when workspaceDir wasn't provided.
        const scanRoot = workspaceDir || cfg.memoryDir;
        let scannedFiles = [];
        try {
          scannedFiles = await scanOpenClawMemoryDir(scanRoot, { logger: api.logger });
        } catch (error) {
          const message = error?.code === "ENOENT"
            ? `OpenClaw workspace directory not found: ${scanRoot}`
        ...
        // When the user selected specific paths, augment the scan with any
        // selections that the recursive scanner skipped (private/ dirs and
        // private-*.md files). The local-server endpoint has already
        // verified the user explicitly confirmed those paths via
        // confirmPrivate=true, so it's safe to upload them here.
        if (filterPaths instanceof Set && filterPaths.size > 0) {
          const missingSelected = [];
          for (const fp of filterPaths) {
            if (!scannedPathSet.has(fp)) missingSelected.push(fp);
          }
          if (missingSelected.length > 0) {
            const adhocRecords = await scanSelectedMarkdownFiles(
              scanRoot,
              missingSelected,
              { logger: api.logger },
            );
            for (const record of adhocRecords) {
              ...
              scannedPathSet.add(record.filePath);
            }
          }
        }
        ...
        // Privacy gate for non-targeted triggers: don't silent-replace any
        // profile-class file (review/private). The user explicitly opted
        // into upload via sync-selected/reextract-selected (those triggers
        // are NOT in NON_TARGETED_TRIGGERS), where each path has already
        // gone through the confirmable check at the endpoint level.
        if (NON_TARGETED_TRIGGERS.has(trigger)) {
          files = files.filter((file) => {
            const { privacyLevel } = classifyFile(file.relativePath);
        ...
        api.logger?.info?.(
          `[echo-memory] sync ${stopped ? "stopped" : "complete"}: scanned=${summary.scanned_file_count} selected=${summary.pending_file_count} processed=${summary.file_count} new_memories=${summary.new_memory_count} skipped=${summary.skipped_count} failed=${summary.failed_file_count}`,
        );
        return state;
      } catch (error) {
