...
export function createSyncRunner({ api, cfg, client, fallbackStateDir = null, stableStateDir = null }) {
  ...
  async function runSync(trigger = "manual", filterPaths = null, options = {}) {
    ...
    const encryptionKeyBase64 = normalizeEncryptionKeyBase64(options?.encryptionKeyBase64);
    if (activeRun) {
      return activeRun;
    }
    ...
    activeRun = (async () => {
      ...
      try {
        ...
        let encryptionStatusError = null;
        if (typeof client?.getEncryptionConfig === "function") {
          try {
            encryptionStatus = await client.getEncryptionConfig();
          } catch (error) {
            encryptionStatusError = error;
          }
        }
        if (encryptionStatusError) {
          const message = "Could not verify Echo encryption status. Sync was stopped to avoid uploading plaintext to an encrypted account.";
          emitProgress(buildProgressPayload({
            phase: "failed",
            runId,
            trigger,
        ...
        if (encryptionStatus?.enabled === true && !encryptionKeyBase64) {
          const message = "Echo encryption is enabled. Unlock encryption in the local UI before syncing.";
          emitProgress(buildProgressPayload({
            phase: "failed",
            runId,
            trigger,
