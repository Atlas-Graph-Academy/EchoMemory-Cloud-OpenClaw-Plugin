 ...
 *  Keep in sync with SKIP_DIRS in local-server.js.
 *  System dirs under ~/.openclaw/ (_disabled-plugins, state, extensions,
 *  credentials, devices, flows, identity, locks) hold plugin runtime data,
 *  not user memories — scanning them caused noisy ENCRYPTION_KEY_REQUIRED
 *  failures and accidentally synced our own state files. */
...
/**
 * Split a markdown blob into sections by `## ` headings and hash each one.
 *
 * The state file stores these alongside the whole-file `contentHash` so the
 * UI can answer "which section changed?" without keeping the prior content
 * around — just compare the new hashes vs the last-synced hashes.
 *
 * Pre-section content (before the first `## `) lives under the synthetic
 * key "__intro__" so files with no headings still produce a usable
 * fingerprint. Duplicate section titles overwrite each other; that's
 * acceptable since we display the actual diff against cloud content,
 * not against these hashes.
 */
export function computeSectionHashes(content) {
  ...
  for (const line of lines) {
    ...
    if (m) {
      sections[currentTitle] = sha256Hex(normalizeContentForHash(currentLines.join("\n")));
  ...
  sections[currentTitle] = sha256Hex(normalizeContentForHash(currentLines.join("\n")));
  // Drop the synthetic intro key when it has no real content (file starts
  // with `## ` immediately) — keeps the hash table honest.
  if (sections.__intro__ === sha256Hex(normalizeContentForHash(""))) {
...
async function buildWorkspaceFileRecord(workspaceDir, filePath, stats = null, content = null, options = {}) {
  ...
  return {
    ...
    h2Count,
    contentHash,
    sectionHashes: computeSectionHashes(resolvedContent),
  ...
  for (const filePath of absolutePaths) {
    ...
    try {
      ...
      records.push({
        ...
        sectionHashes: computeSectionHashes(content),
      });
    } catch (error) {
      logger?.warn?.(
