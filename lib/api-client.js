...
export function createApiClient(cfg) {
  ...
  const webGraphLinkUrl = `${cfg.baseUrl}/api/openclaw/v1/web-graph-link`;
  const publicGraphStatusUrl = `${cfg.webBaseUrl}/api/public-graph/my-status`;
  const publicGraphPublishUrl = `${cfg.webBaseUrl}/api/public-graph/publish`;
  ...
  async function createWebGraphLink() {
    if (!cfg.apiKey) {
      throw new Error("Missing Echo API key");
    }
    return requestJson(
  ...
  async function getPublicGraphStatus() {
    if (!cfg.apiKey) {
      throw new Error("Missing Echo API key");
    }
    return requestJson(
      publicGraphStatusUrl,
      {
        method: "GET",
        headers: buildHeaders(cfg.apiKey),
      },
  ...
  async function publishPublicGraph() {
    if (!cfg.apiKey) {
      throw new Error("Missing Echo API key");
    }
    return requestJson(
      publicGraphPublishUrl,
      {
        method: "POST",
        headers: buildHeaders(cfg.apiKey),
        body: JSON.stringify({}),
