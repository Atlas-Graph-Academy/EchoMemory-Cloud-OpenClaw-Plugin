function normalizeTopic(rawTopic) {
  ...
  if (/(install|config|setup|env|memorydir|baseurl|webbaseurl)/.test(topic)) return "setup";
  ...
  return {
    signup: {
      ...
      lines: [
        ...
        "2. Log in to the web app and create a developer API key at `https://www.iditor.com/api`.",
        "3. Put that `ec_...` key into the plugin config as `apiKey`.",
        "4. Recommended scopes: `ingest:write` for sync and `memory:read` for retrieval.",
      ],
    },
    setup: {
      title: "Install and config",
      lines: [
        `- Required config: \`baseUrl\` (current backend, now \`${cfg.baseUrl}\`) and \`apiKey\`.`,
        `- Optional config: \`webBaseUrl\` (current web app, now \`${cfg.webBaseUrl}\`), \`memoryDir\`, \`autoSync\`, \`syncIntervalMinutes\`, \`batchSize\`, \`requestTimeoutMs\`.`,
    ...
    commands: {
      ...
      lines: [
        ...
        `- \`${commandLabel} graph\` opens the private graph login page and \`${commandLabel} graph public\` opens the public memories page.`,
